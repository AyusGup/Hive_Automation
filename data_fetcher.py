...
# Replace with your Steem credentials
STEEM_NODE = "https://api.hive.blog"
PRIVATE_KEY = "5JBtftx2AABs84Rf8Unee9rQd7sgrDthZHdhy5GVc7W2eS6vQZC" 
ACCOUNT_NAME = "ag8350961" 
BufferError = 0.02
...
def fetch_recent_trades(last_fetch_time=None, limit=100, batch_size=1000):
    """
...
def predict_next_prices_with_history(buy_trades, sell_trades, last_buy_price, last_sell_price, interval_minutes=2):
    """
    Predict next buy and sell prices based on trends and historical trade prices.
    """
    buy_prices = [trade['price'] for trade in buy_trades]
    sell_prices = [trade['price'] for trade in sell_trades]

    if len(buy_prices) >= interval_minutes:
        predicted_buy_price = sum(buy_prices[-interval_minutes:]) / len(buy_prices[-interval_minutes:])
    else:
        predicted_buy_price = 0

    if len(sell_prices) >= interval_minutes:
        predicted_sell_price = sum(sell_prices[-interval_minutes:]) / len(sell_prices[-interval_minutes:])
    else:
